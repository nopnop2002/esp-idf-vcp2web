...
static bool handle_rx(const uint8_t *data, size_t data_len, void *arg)
{
	...
	ESP_LOGD(TAG, "my_json_string=[%s]",my_json_string);
	int my_json_length = strlen(my_json_string);
	cJSON_Delete(root);
	size_t sended = xMessageBufferSendFromISR(xMessageBufferRx, my_json_string, my_json_length, NULL);
	ESP_LOGD(TAG, "my_json_length=%d",my_json_length);
	ESP_LOGD(TAG, "xMessageBufferSendFromISR sended=%d", sended);
...
extern "C" void cdc_acm_vcp_task(void *pvParameters)
{
    ...
    // Allocate memory
    char* messageBuffer = (char *)malloc(xItemSize+1);
    if (messageBuffer == NULL) {
        ESP_LOGE(TAG, "messageBuffer malloc Fail");
        while(1) { vTaskDelay(1); }
    }
    char* txBuffer = (char *)malloc(xItemSize+1);
    if (txBuffer == NULL) {
        ESP_LOGE(TAG, "txBuffer malloc Fail");
        while(1) { vTaskDelay(1); }
    }
	...
	while (true) {
		...
		*/
		...
		while(1) {
			size_t received = xMessageBufferReceive(xMessageBufferTx, messageBuffer, xItemSize, 100);
			...
			if (received > 0) {
				...
				if (cJSON_GetObjectItem(root, "id")) {
					...
					ESP_LOGI(TAG, "payloadLen=%d payload=[%s]",payloadLen, payload);
					if (payloadLen) {
						ESP_LOG_BUFFER_HEXDUMP(TAG, payload, payloadLen, ESP_LOG_INFO);
						strncpy(txBuffer, payload, payloadLen);
						txBuffer[payloadLen] = 0x0a;
						ESP_LOGI(TAG, "Sending data through CdcAcmDevice");
						ESP_LOG_BUFFER_HEXDUMP(TAG, txBuffer, payloadLen+1, ESP_LOG_INFO);
						ESP_ERROR_CHECK(vcp->tx_blocking((uint8_t*)txBuffer, payloadLen+1));
					}
				}
				cJSON_Delete(root);
			}
